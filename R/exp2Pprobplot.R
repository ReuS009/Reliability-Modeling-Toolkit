...
probplot.exp2P <- function(data,pp,xlabel1) {
  # Check if X is there
  if(missing(xlabel1)) {
    xlabel1<-"X"
  }
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      ttfc_list[[i]]<-probplotparam.exp2P(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,2])
  ...
  legend(min(ttfcrange), -log(1-0.99,), legend=c("Data", "2P Exponential best fit line"),
