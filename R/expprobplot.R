...
probplot.exp <- function(data,pp,xlabel1) {
  # Check if X is there
  if(missing(xlabel1)) {
    xlabel1<-"X"
  }
  ...
  if (!is.null(dim(databystress))){
    ...
    for(i in 1:length(databystress)){
      ...
      ttfc_list[[i]]<-probplotparam.exp(xiRFblock_list[[i]][,1],xiRFblock_list[[i]][,2])
  ...
  #sprintf("Exponential fit expression(lambda) = %3.4f", ttfc[[3]][,1])
  #lam<-ttfc[[3]][,1]

  legend(min(ttfcrange), -log(1-0.99,), legend=c("Data", "Exponential best fit line"),
